...
public:
  ...
  : Node("gripper_action_adapter", options)
  {
    ...
    this->declare_parameter("max_gripper_width", 0.07);  // Maximum gripper opening (7cm)
    ...
    // Calculate joint limits from max_gripper_width (symmetric joints)
    joint_max_position_ = max_gripper_width_ / 2.0;
    ...
    RCLCPP_INFO(this->get_logger(), "Gripper Action Adapter initialized");
...
private:
    ...
    const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const ParallelGripperCommand::Goal> goal)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Goal position: %.4f m, max_effort: %.2f N", goal_position, goal_effort);
  ...
  {
    // Convert parallel gripper command to individual joint positions
    // gripper_width is the desired distance between gripper fingers
    // left joint: moves from 0 to +width/2
    // right joint: moves from 0 to -width/2
    double target_left_joint_position = gripper_width / 2.0;
    double target_right_joint_position = -gripper_width / 2.0;

    // Clamp to valid joint ranges (symmetric)
    target_left_joint_position = std::clamp(target_left_joint_position, 0.0, joint_max_position_);
    target_right_joint_position =
      std::clamp(target_right_joint_position, -joint_max_position_, 0.0);

    // Create and publish position command
    auto position_cmd = std_msgs::msg::Float64MultiArray();
    position_cmd.data = {target_left_joint_position, target_right_joint_position};

    RCLCPP_INFO(
      this->get_logger(), "Sending position commands: left=%.4f, right=%.4f",
      target_left_joint_position, target_right_joint_position);
  ...
  double joint_max_position_;  // Calculated as max_gripper_width_ / 2.0
};
...
int main(int argc, char * argv[])
{
  ...
  RCLCPP_INFO(node->get_logger(), "Starting Gripper Action Adapter node");
